...
class LogicalCount(LogicalAggregateExpression):
    def __init__(self, expression: LogicalExpression):
        super().__init__(name="COUNT", expression=expression)

    def to_field(self, plan: LogicalPlan) -> Field:
        return Field(
            name=self.expression.to_field(plan).name, data_type=ArrowTypes.Int32
        )

    def to_physical_expression(self, plan: LogicalPlan) -> PhysicalAggregateExpression:
        return PhysicalCount(self.expression.to_physical_expression(plan))
